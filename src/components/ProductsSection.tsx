...
export default function ProductsSection({ whatsappNumber, productMessage }: ProductsSectionProps) {
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products?.map((product) => (
              <ProductCard key={product.id} product={product} whatsappNumber={whatsappNumber} productMessage={productMessage} />
            ))}
          </div>
        )}
      </div>
